...
simulate_game <- function(tree, real_word, silent=TRUE) {
  num_plays <- 0
  moves <- rep("", 10)
  while (!tree$isLeaf) {
    move <- tree$move
    ...
    result <- simulate_outcome(move, real_word, encoded=TRUE)
    tree <- tree[[result]]
    num_plays <- num_plays + 1
    moves[num_plays] <- move
  }
  if (!is_null(tree$move)) {
    move <- tree$move
    if (!silent) {
      print("Tried...")
      print(move)
  ...
  if (!silent) {
    print(paste("Won on", num_plays, "plays"))
  }
  list(
    num_plays = num_plays,
    moves = moves[moves != ""]
...
load_assistant <- function(filename) {
  FromListExplicit(yaml.load(read_yaml(filename)))
}

load_assistants <- function(filenames) {
  assistants <- list_along(filenames)
  names(assistants) <- names(filenames)
  for (name in names(filenames)) {
    assistants[[name]] <- load_assistant(filenames[[name]])
  }
  assistants
}
